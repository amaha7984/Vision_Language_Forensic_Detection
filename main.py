...
torch_kmeans_path = '/aul/homes/amaha038/DetectionGeneratedImages/LASTED/torch_kmeans/src'
sys.path.append(torch_kmeans_path)
...
class ImageDataset(Dataset):
    def __init__(self, data_root, train_file, data_size=256, val_ratio=None):
...
# Helper function for AP calculation with validation
def compute_ap(gt_labels_list, prob_labels_list):
    if len(gt_labels_list) > 0 and len(prob_labels_list) > 0:
        try:
            ap = average_precision_score(gt_labels_list, prob_labels_list)
            return ap
        except Exception as e:
            logger.warning(f"Failed to calculate AP: {e}")
            return None
    else:
        logger.warning("AP calculation skipped due to empty or mismatched lists.")
        return None
...
def validation_cluster(model, args, test_file='', data_loader=None):
    ...
    if 0 not in label2features_dict or 1 not in label2features_dict:
        print("One or both labels are missing in the validation set")
    ...
    ap = compute_ap(gt_labels_list, prob_labels_list) or 0 
...
def main(args):
    ...
    if args.test_file == '':
        test_file_list = [
            # (Test_file.txt, nickname of the dataset)
            ('annotation/Test.txt', 'MapDataset')
        ]
    else:
        test_file_list = [
    ...
    model = getattr(importlib.import_module('model'), args.model)(num_class=args.num_class, simclr_checkpoint_path=args.simclr_checkpoint)
    ...
    for param in model.module.simclr_encoder.parameters():
        param.requires_grad = False  # Set to True if fine-tuning"""
    ...
    for epoch in range(args.epoches):
        ...
        if args.isTrain == 1:
            ...
            if val_auc is not None and val_acc is not None and val_ap is not None:
                logger.info('Score: Validation AUC: %5.4f, Acc: %5.4f, AP: %5.4f' % (val_auc, val_acc, val_ap))
            else:
                logger.info('Skipping validation scores due to missing labels in the validation set.')
...
if __name__ == '__main__':
    ...
    conf.add_argument("--data_root", type=str, default='data/Test/',
                      help="The root folder of training set.")
    conf.add_argument("--train_file", type=str,
                      default='annotation/Train.txt')
    conf.add_argument("--test_file", type=str,
                      default='annotation/Test.txt')
    ...
    conf.add_argument('--epoches', type=int, default=500, help='The training epoches.')
    ...
    conf.add_argument("--simclr_checkpoint", type=str, default='./SIMCLR/Weights/best_simclr_checkpoint.ckpt',
                      help="Path to the trained SimCLR checkpoint.")
